...
% environment to display a page in landscape in the final pfd
...
\authornote{M. K. Healey is the sole author of this article and is responsible for its content. I thank Mitchell Uitvlugt, Kim Fenn and SOME OTHER PEOPLE for helpful discussions.
...
% setup some commands to make chaning text from study to study easy
\newcommand\listlength{16} % words per list 
\newcommand\presrate{4 seconds} % time per word
\newcommand\isi{1 second} %
\newcommand\DFRDelay{16 seconds} % lenght of distractor at end of study
...
% But are contiguity-generating processes directly linked to dilebrate encoding control processes, or do the two just happen to be confounded in the size judgment tasks? 
...
% % Particiapnts in the task condition are likely to engage in they type an extent of processing needed to make the judgment, but no more. subjects in the intent ocndition likely engage in a host of other encoding processes. Therefore . Or it could be that the extra processing 
...
%todo: need to make the point clearer: control processes are critical, but it is not as simple as adhoc stratgies.
...
% Mention other factor that could differ between intent and no intent and might explain the residule betwen intent and no intent:
%  Such as the time on the processing taks. When subjects are trying to remember the items they are likely to engage in enought processing of each item to complete the task, but no more. Therefore, if subjects can finish . 
%  Contrast this with what people are likely to do when dileberately studying words. tehy are paying a great deal of attention to specific words, as we have argued in descriptsion of the retreived context framework, subjects are allowing each new stimuli to trigger a cascade of episodic and semantic associaitons which generates a distinct pattern of mental activity (i.e., mental context), that is specific to that item. Then when the next item is studies it activates a different set of associations and this new pattern of activity partially, but not totally, replaces the prior pattern. That is, mental context drifts in an autocorrelated fashion. Critically, this does not depend on any deliberate relational encoding, this drift of context will happen naturally as a biproduct of thinking deeply about individual items in isolation. Therefore, if a particilar task encourages this same sort of deep processing of items should drive context drift and create a contiguity effect, \emph{even if there is no intent to encode items}
