\documentclass[jou,natbib]{apa6} %apa 6th edition format
...
\shorttitle{Contiguity in Episodic Memory}
...
%Todo: this does make it sound like the lack of the effect is key.
%Todo: make it align better with the intro about control processes. say that cases where the experimenter exerts a lot of control and the subject little
\abstract{The law of similarity is a key principle of memory: recalling one memory tends to trigger recall of similar memories. The amount of time separating two events is believed to be a powerful determinant of similarity. If so, there should be a strong Temporal Contiguity Effect whereby thinking of one event triggers recall of other events experienced nearby in time. But evidence for this Temporal Contiguity Effect has come almost exclusively from tasks that explicitly ask subjects to memorize items. Are memories naturally organized by temporal proximity when there is no intent to memorize, as is often the case outside the laboratory? Two experiments show that removing the intent to memorize can also remove the Temporal Contiguity Effect. A third experiment shows that it is not the intent to memorize per se, but rather how subjects process information while incidentally learning it that generates temporal contiguity.}
...
%Thinking about the deep semantic meaning of an item is more effective than thinking about its phonological characteristics \cite{craik}. 
...
% More precisely, the TCE seems to depend on processing that occurs when trying to memorize a word but not when simply judging if a word's referent would fit in a shoebox or through your front door. 

% But are contiguity-generating processes directly linked to deliberate encoding control processes, or do the two just happen to be confounded in the size judgment tasks? 

% this critical processing inseparable from intent to encode

% THIS COULD POSE THE CENTRAL QUESTION MORE CRISPLY: But is the critical difference between these two conditions the deliberate engagement of encoding control processes that are inseparable from intent to encode. Or is it that the judgment tasks just reduces the TCE in a way that is not directly related to intent to encode?

% Are these critical processes inseparable from intent to encode (i.e., can you have a TCE without intent), or are they processes that 



% For example, under most models \citep{HealEtal16}, the TCE depends on the memory representations of items that are studied close together being more similar to each other than they are to the representation of items studied far apart. But both the Shoebox and Door tasks make it likely that subjects are thinking of a shoebox or their home as they study each item. That is, they are maintaining a common mental representation through the list presentation. Therefore the items are likely to form associations with this representation, increasing the similarity of items that are separated by distant lags and potentially attenuating the TCE. By contrast, under explicit encoding instructions subjects are likely to engage in additional processing of the items beyond that needed to make a size judgment. This extra processing might decrease the similarity of successive items, mitigating the effect of associating each item with a common representation. This is just one, admittedly speculative, example of how an incidental encoding task might decrease the TCE in a way that is not due to the lack of intent to encode per se.      

% % Participants in the task condition are likely to engage in they type an extent of processing needed to make the judgment, but no more. subjects in the intent condition likely engage in a host of other encoding processes. Therefore . Or it could be that the extra processing 

% But we can test a more general prediction: If the TCE is intimately tied up with the intent to encode, it should be difficult, or perhaps impossible, to find an incidental encoding task that produces a TCE. Whereas if the TCE depends on intent to encode only to the extent that intent makes different types of processing more or less likely, then it should be very easy to find incidental encoding tasks that produce a TCE. Experiment 3 tests this predicting by examining five different encoding tasks that differ in the type of processing required.





% AND OR: Therefore, whereas mental features representing size may remain activated from item to item, activity along other dimensions is free to drift from item to item. If we think of this in terms of three dimensions, the Scenario Task may force context to return to the same X and Y coordinates for each item, allowing only Z to vary. Whereas the size task constrains only a single dimension allowing the other two to vary. Therefore, mental context may change more from item to item, producing a larger contiguity effect.



% and 2) determine if different types of processing can produce contiguity without intent to encode.
...
These results show that the TCE is powerfully influenced by processing participants choose to engage when deliberately encoding information---ex1 and e2. Just as clearly, the results show that the TCE does require these deliberate processes---e3 showed strpmg TCE in five different implicit encoding tasks.

two theoretical contributions. Speaks against models that ascribe to task-specific strategies (see laws paper) points to a general mechanism. But, perhaps more importantly, points to a large gap in current versions of such models. They would have difficulty with the E1 and E2 findings.
...
%todo: need to make the point clearer: control processes are critical, but it is not as simple as adhoc strategies.
...
% There are many differences between the incidental encoding tasks that did and did not produce a TCE and it is not clear which of these differences is critical. This gap highlights two limitations of current memory models.



% Binding each item to a common representation is plausible hypothesis, but it was not rigorously tested here. 
...
% Experiment 1 showed that the TCE can be eliminated under incidental encoding. Experiment 2 replicated this finding. In both of these experiments the incidental encoding task encouraged to keep a specific visual image active through the list, possibly increasing the similarity of memories encoded far apart in time. Experiment 3 used five different implicit encoding tasks that did not share this feature and found a clear TCE in each case. These results show that the TCE is not due to task-specific strategies implemented by controlled encoding process \citep[cf.][]{Hint16}.

% Although these results clearly show that contiguity does not require intent, there are many differences between the incidental encoding tasks that did and did not produce a TCE and it is not clear which of these differences is critical. Binding each item to a common representation is one hypothesis, but it was not rigorously tested here. That is, this initial report stands as an existence proof for temporal contiguity under incidental encoding but also points to a large gap in our understanding of how encoding processes influence contiguity. This gap highlights two limitations of current memory models.

% First, many models of the TCE do not implement encoding control processes \citep[but see][]{LehmMalm13}, and would likely have difficulty accounting for the difference between the Explicit and Incidental conditions. Second, although models have simulated different processing tasks by assigning them distinct contextual states \citep{PolyEtal09}, it is unlikely that this would allow a model to capture the influence of type of processing on the size of the TCE. Some mechanism to allow task demands to modulate parameters such as the rate of mental context drift seems necessary \citep[for a similar point, see][]{HealKaha14}. 

%  that a full model of the TCE must include mechanisms that can modulate how items are processed during encoding. 

% These results confirm that the type of processing subjects choose to engage during encoding powerfully influences how they recall the information and can eliminate the TCE \citep{Hint16,NairEtal17}. But.... positive spin??


% Mention other factor that could differ between intent and no intent and might explain the residual between intent and no intent:
%  Such as the time on the processing tasks. When subjects are trying to remember the items they are likely to engage in enough processing of each item to complete the task, but no more. Therefore, if subjects can finish . 
%  Contrast this with what people are likely to do when deliberately studying words. They are paying a great deal of attention to specific words, as we have argued in description of the retrieved context framework, subjects are allowing each new stimuli to trigger a cascade of episodic and semantic associatons which generates a distinct pattern of mental activity (i.e., mental context), that is specific to that item. Then when the next item is studies it activates a different set of associations and this new pattern of activity partially, but not totally, replaces the prior pattern. That is, mental context drifts in an autocorrelated fashion. Critically, this does not depend on any deliberate relational encoding, this drift of context will happen naturally as a byproduct of thinking deeply about individual items in isolation. Therefore, if a particular task encourages this same sort of deep processing of items should drive context drift and create a contiguity effect, \emph{even if there is no intent to encode items}
