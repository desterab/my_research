...
% Under retrieved context theories, memory for an event is fundamentally mediated by its associations to a temporal context representation. During encoding, context drifts in an autocorrelated fashion such that items presented in temporal proximity become associated with similar contextual states. Then during retrieval, each recalled item reinstates its associated context, providing a strong cue to items associated with similar contexts, which tend to be items studied nearby in the list. That is, event-to-context associations\footnote{For simplicity we refer only to event-to-context associations. But the models generally also include context-to-event associations, which can have different strengths and learning rate parameters.} are what give a memory its episodic character, linking the content of an experience to the temporal context in which it occurred. Does this fundamental link between formation of episodic memories and formation of new event-to-context associations mean that we continuously form event-context associations and that the strength of these associations is unmodulated by factors such as our intent to form new memories \citep{Hint15}? It does not, for several reasons. 

% First, one of the most basic observations about memory is that it is imperfect---we often fail to encode or fail to retrieve even important events. In other words, saying temporal associations are fundamental to forming new episodic memories is not the same as saying temporal associations are formed easily. Instead, it is saying that the success of episodic encoding and the level of contiguity are inherently correlated such that, all else being equal, they will rise and fall together.

% Second, from a theoretical perspective, there is a large potential cost to forming an association---it generates interference, a dense web of associations that would make it difficult to target specific events. Or as \citet{Hint15} puts it, continually forming temporal associations of equal strength would produce "a virtual spaghetti-plate of irrelevant connections." We agree, but this is not to say that such spaghetti-plates are non-existent; just that using memory optimally may entail judiciously regulating the formation of new associations \citep{HashEtal07}. Indeed, it has been argued that failure to modulate the formation of new associations based on task demands is one of the chief sources of memory deficits seen in aging \citep{HealEtal14b,HashZack88} and a variety of developmental disorders \citep{Nigg00,Demp91}. This theoretical point is born out by data suggesting that although temporal associations can form without intentionality \citep{Unde83,CrowEtal90}, intentionality has a powerful influence \citep{SpeaEtal64,Kepp66,Gree87,CampEtal10,BissEtal13}. Thus, it is far from obvious that healthy, young, intelligent college students should form strong temporal associations under all conditions.

% The idea that people can regulate the extent to which new associations are formed fits quite comfortably within the retrieved context modeling framework. In most implementations of the model, the strength of newly formed temporal associations is a parameter that can, in principle, go to zero, in which case the model would form no new associations. We have previously argued that improvements in performance with practice may result from subjects fine-tuning such parameters \citep{HealKaha14}. Similarly, we have used the ability to modulate association formation parameters to test the theory that age differences in memory result from impairments in forming item-context associations \citep{HealKaha15}. And an extension of the model \citep{Sede12} explicitly assumes that the extent to which associations are formed is a function of how predictable the current event is based on the preceding events \citep[i.e., surprising events are encoded more strongly. For a similar mechanism in positional coding models see][]{FarrLewa02}.

% Therefore, we predict that the contiguity effect would be strongly attenuated by any manipulation that also attenuates formation of new episodic memories, such as incidental encoding instructions \citep{Hint15,NairEtal13}. Given appropriately high-quality data, this prediction can be tested quantitatively by allowing these parameters to vary from moment-to-moment to simulate fluctuations in attention, or to yoke the value of these parameters to task variables such as incidental versus explicit encoding instructions.
