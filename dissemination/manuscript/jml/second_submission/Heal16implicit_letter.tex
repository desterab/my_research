...
\item
	...
	On page~\pageref{TODO-8} I discuss several studies that have tested for serial order after incidental encoding \cite{Nair91, Nair90b, Burn96, SerrNair93}. I note that these studies provide strong evidence that temporal information is encoded, but that the convering evidence form the current work is valuable given this interpretation is not without critics \cite{Hint16} or counter examples that have been taken by some scholars as showing minimal access to temporal information \cite{BradGlen83,GlenBrad79,HintEtal75,HintBloc73}. I rephrase the question of the current manuscript to focus on whether or not subjects spontaneously use temporal information to guide memory search on a free recall task.
...
\begin{enumerate}
	...
	% Espically in this case where a small effect has big theoritical implications, I think it is espically important to focus on whether or not the effect size is consistent across multiple samples---Figure 11 shows that it is.

	% I think avoidng p-values and focusing on effect sizes across multiple conditions might be espically important in this case given that we are dealing with a small effect that has big theoritical implications. Figure 11 shows that size of the effect is quite consistent across conditions, suggesting that attempts to replicate 

	% but because of its small size, it is going to be hard to detect using a hypothesis test in

	%  and they also avoid some of the common criticisms of hypothesis testing. %I think avoidng p-values and focusing on effect sizes across multiple conditions might be espically important in this case given that we are dealing with a small effect that has big theoritical implications which makes the dangers of p-hacking salient.
