...
\item
	...
	As requested, I have included a discussion of how the size of the TCE in the current study compares to that observed in the previous work. This discussion includes an analysis of subject's second lists. (see my response to the Action Letter for details). TODO: BUT WHAT ABOUT ASYMETERY
...
\begin{enumerate}
	...
	The reviewer pointed out that previous studies that followed incidental encoding with direct tests of temporal order memory show that temporal information survives incidental encoding. On page \pageref{TODO-8} of the revision I discuss these findings along with the more recent demonstration of this from the order reconstruction task in experiment 3 of Nairne et al. (2017), while also noting that this interpretation of the data is not without its critics (Hintzman 2015). Thus, I see the contributions of the current manuscript as 1) providing converging evidence for the existence of incidentally encoded temporal associations and 2) showing that subjects spontaneously use these associations to guide memory search even when not prompted to do so with a direct order test. Moreover, I completely agree with the reviewer that the question is now "under what conditions do people use this information strategically to drive recall?" I hope this initial manuscript can provide a foothold for a research programing aimed that this question by answering the more basic question of whether people ever spontaneously use this temporal information. 

	Say something about The new experiment's serial condition!!

	This is, I think the topic for an enitre research pro I think the current paper provides a starting point by showing that the spontaneously use this information
	...
	CITE NEATH 97 to talk about the influence of providing the item in overcomming output interference.
