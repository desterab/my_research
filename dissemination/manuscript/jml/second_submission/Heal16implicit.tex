...
TODO:
For example, imagine that a participant recalled the words from serial positions 5, 6, 8 in that order. The actual temporal factor score for that recall sequence is XX. We would build a distribution of the temporal factor score expected by chance by taking these recalled serial postions and randomly shuffling them. For example, rearanging the sequence to 6, 5, 8 gives a temporal facor score of XX. Rearanging them to 
